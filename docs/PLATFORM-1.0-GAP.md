...
| 6. Design System | PARTIAL | CSS-tokens, gemensam sajtmall, leftover skip-to-content på sajt, apprum och gäst, leftover 404 och error ur IdP-nycklar | Tokenpaket, komponentbibliotek, produkt-DNA-regler, a11y-svit |
...
| Indexerbar | PARTIAL | robots tillåter sajt, blockerar apprum + `/platform` + `/api/`. HTML `noindex` på samma rum plus gästlänk, bekräftelse, leftover 404 och Identity-HTML |
...
| Locale | PARTIAL | `html lang` från `pd_locale`, inklusive leftover Identity-logout. Leftover dokumentationstermer ur godkända översättningar. Inga locale-URL:er |
