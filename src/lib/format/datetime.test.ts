...
import { formatDate, formatSwedishDate, formatSwedishDateTime } from "./datetime.ts";
...
describe("Swedish clock copy", () => {
  it("renders a Stockholm wall time instead of a raw ISO string", () => {
    ...
    expect(formatDate(iso, "en")).toMatch(/Sep/i);
    expect(formatDate(iso, "sv")).toMatch(/sep/i);
  });
