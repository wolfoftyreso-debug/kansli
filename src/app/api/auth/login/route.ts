...
import { leftoverColorSchemeMeta } from "@/lib/platform/theme-chrome";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return new NextResponse(
      `<!doctype html><html lang="${localeTag(locale)}"><meta charset="utf-8">${APP_ROOM_VIEWPORT_META}${leftoverColorSchemeMeta()}${appRoomRobotsMeta()}<title>${t(locale, "idp.loginUnavailable")}</title><body style="font-family:system-ui;max-width:36rem;margin:3rem auto;padding:0 1rem"><h1>${t(locale, "idp.loginUnavailable")}</h1><p>${t(locale, "idp.loginUnavailableBody", { home: `<a href="/">${home}</a>` })}</p></body></html>`,
